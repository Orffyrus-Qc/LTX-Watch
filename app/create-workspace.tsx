        ...
        <div className="create-composer">
          ...
          <div className={`create-card director-card ${draft.directorMode ? 'enabled' : ''}`}>
            ...
            {draft.directorMode && <div className="director-editor">
              ...
              <label className={`director-sheet ${draft.ingredientsReferencePath ? 'uploaded' : ''}`}><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ''; if (file) void uploadContext([file], 'ingredientsReferencePath'); }} /><ImagePlus size={18} /><span><b>{pending === 'ingredientsReferencePath' ? 'Uploading reference sheet…' : draft.ingredientsReferencePath ? 'Ingredients reference sheet ready' : 'Upload Ingredients reference sheet'}</b><em className="director-sheet-requirement"><CircleAlert size={11} /> Required for Director generation · Import at least one reference photo</em><small>Place only important character, wardrobe, prop, or location references side by side. The sheet guides every frame and does not become frame one.</small></span></label>
