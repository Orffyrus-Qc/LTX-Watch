...
3. Optionally enable local prompt enhancement, camera direction, motion intensity, visual style, and synchronized, ambience-only, soundtrack-replacement, or stripped audio. Enhancement uses the dedicated Gemma `e2b-it` model and may creatively rewrite a short request; leave it off when literal wording matters most.
