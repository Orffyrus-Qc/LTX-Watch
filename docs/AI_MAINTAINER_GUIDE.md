...
3. Match prompt, enhancer, duration, resolution, seed, and frame rate by subgraph labels/names. Do not hardcode node IDs or model filenames. Preserve the generic `/object_info/<class_type>` compiler for input ordering and dynamic nodes.
   Preserve conservative combo reconciliation: keep valid template values, permit a renamed live model only when family and precision/format markers produce one unique match, and leave ambiguous values invalid. Treat `prompt_enhance_model` as a semantic role: only a dedicated `gemma4_e2b_it_*` checkpoint may satisfy it, never the main `with-proj-ltx-2.5` encoder. A sole role-compatible enhancer variant may replace a stale template precision suffix; do not generalize that exception to transformer, VAE, main text-encoder, or upscaler inputs.
4. The compiler supports multiple nested subgraphs. If graph input/output sentinel semantics, node input ordering, or subgraph-instance widget layout changes, add a sanitized official-structure fixture and update recursive expansion deliberately. Do not flatten by node ID or silently choose a graph.
...
For Director mode, separately verify all upstream contracts: `kijai/ComfyUI-PromptRelay` and its `PromptRelayEncode` API inputs/outputs; Lightricks' exact ComfyUI-LTXVideo node, LTX 2.5 Ingredients workflow, and semantic patch chain; the installed node's Kornia API compatibility; and the Lightricks Ingredients IC-LoRA filename/license. Preserve server-side capability discovery and never accept a workflow path, model filename, node class, or graph from the browser. Patch by unique class plus consumer relationship, fail on ambiguity, and never fall back to ordinary Create. Preserve the native LTX conditioning path and its audio/video embedding metadata; Prompt Relay's patched model owns temporal scheduling, while the native encoder receives the identical combined global/local token sequence. Keep the reference sheet private and job-scoped. If the official workflow begins exposing explicit width/height, add fixture coverage before enabling the normal resolution selector; until then it truthfully follows sheet aspect at the workflow's 544-pixel short edge. Real Director validation requires explicit user direction and an idle GPU.
