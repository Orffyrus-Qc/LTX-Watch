...
The compiler expands any number of nested ComfyUI subgraphs by definition ID, maps exposed inputs by `label`/`name`, resolves graph-input and graph-output sentinels recursively, and queries the isolated loopback server's `/object_info/<class_type>` route to preserve ComfyUI's current required/optional input order. It handles `COMFY_AUTOGROW_V3` and `COMFY_DYNAMICCOMBO_V3`, follows UI-only reroutes, ignores notes/resolution selectors and obsolete exposed inputs that no inner link consumes, and constrains `SaveVideo.filename_prefix` to `video/ltx-watch-create/<job-id>`. Node numeric IDs are not an API and must not become semantic constants.

### Director Prompt Relay contract

Director uses explicitly detected local dependencies:

- `custom_nodes/ComfyUI-PromptRelay` from `kijai/ComfyUI-PromptRelay`
- Lightricks' official `custom_nodes/ComfyUI-LTXVideo` and its `example_workflows/2.5/LTX-2.5_ICLoRA_Ingredients_Single_Stage_Distilled.json`
- `models/loras/ltx-2.3-22b-ic-lora-ingredients-0.9.safetensors`
- Kornia in the configured ComfyUI Python environment. The currently verified ComfyUI-LTXVideo revision imports pyramid helpers removed by Kornia 0.8.3, so Kornia 0.8.2 is the verified compatibility pin. The capability check reads package metadata only and relaxes this bound if a later node revision no longer imports that legacy API.
...
After recursive compilation, the runner identifies patch points by unique node classes and verified graph relationships: `LTXICLoRALoaderModelOnly`, `CLIPLoader`, `EmptyLTXVLatentVideo`, and the `LTXVConditioning` feeding `LTXAddVideoICLoRAGuide`. It inserts `PromptRelayEncode`, routes the patched model to the existing model consumers, applies the bounded IC-LoRA strength, and keeps conditioning on the official native encoder chain. The native encoder receives the exact global-plus-local token sequence used by the relay; this preserves LTX 2.5's unprocessed audio/video embedding metadata, which is lost if Prompt Relay's generic conditioning output is connected directly. Ambiguous or missing patch points fail visibly. There is no fallback to T2V/I2V/FLF2V.
...
Static template enum values can lag renamed local model files. For ordinary ComfyUI combo inputs, including constants linked through an exposed subgraph input, the compiler preserves a value that still exists. If it is missing, it may select a live value only when the filename family plus precision/format markers such as `int8` and `convrot` leave exactly one candidate, or when the node exposes only one choice. Semantic roles impose stricter families: `prompt_enhance_model` accepts only a dedicated `gemma4_e2b_it_*` checkpoint; the main LTX text encoder accepts only a projected `with-proj-ltx-2.5` checkpoint; and the checkpoint lookup used by `GemmaAPITextEncode` accepts only an LTX transformer, never an unrelated checkpoint such as SAM. When exactly one role-compatible variant is installed, that identity may take precedence over a stale template precision suffix. Upload selectors such as `LoadImage.image` are excluded from reconciliation: they are file identities, not model enums. Ambiguous substitutions remain invalid and fail visibly; the adapter must never choose an arbitrary checkpoint or replace an uploaded reference with a sample input.

### Verified Director baseline

On 2026-08-27, the isolated Director adapter completed a private smoke render against the detected public dependencies and official example Ingredients sheet. The output contained 121 H.264 frames at 24 fps, AAC audio, and a 5.04-second duration; an independently decoded midpoint frame was valid. No Project, Studio, album-worker, or user queue was used. This validates workflow compilation and media output, not a guarantee of creative quality or identity persistence.

Current ComfyUI `LoadImage.INPUT_TYPES` enumerates immediate files in the input root rather than nested Create folders. The runner therefore stages first/last references under unique job-scoped filenames directly in `<comfyRoot>/input`, compiles those exact names, and removes the copies after the isolated server stops. Keep the source references in the ignored per-job runtime and retain the root-stage/cleanup regression test whenever ComfyUI changes its input schema.
