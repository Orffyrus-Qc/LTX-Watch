...
Static template enum values can lag renamed local model files. For ordinary ComfyUI combo inputs, including constants linked through an exposed subgraph input, the compiler preserves a value that still exists. If it is missing, it may select a live value only when the filename family plus precision/format markers such as `int8` and `convrot` leave exactly one candidate, or when the node exposes only one choice. Upload selectors such as `LoadImage.image` are excluded from this reconciliation: they are file identities, not model enums. Ambiguous model substitutions remain invalid and fail visibly; the adapter must never silently switch precision, choose an arbitrary checkpoint, or replace an uploaded reference with a sample input.
