...
Static template enum values can lag renamed local model files. For ordinary ComfyUI combo inputs, the compiler preserves a value that still exists. If it is missing, it may select a live value only when the filename family plus precision/format markers such as `int8` and `convrot` leave exactly one candidate, or when the node exposes only one choice. Ambiguous substitutions remain invalid and fail visibly; the adapter must never silently switch precision or choose an arbitrary checkpoint.
