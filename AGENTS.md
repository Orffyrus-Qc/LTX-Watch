...
- Never call `/api/quit` during an automated check; it stops the local Watch UI/bridge. It must never pause, interrupt, or kill the LTX/ComfyUI generator.
...
`create-core.mjs` owns pure option, prompt, seed, Director timeline, draft, and queue invariants. `app/create-workspace.tsx` owns the original-video UI. `local-server.mjs` validates capabilities, registered roots, uploads, private jobs, queue transitions, and the shared generation launch claim. `scripts/ltx-create-runner.py` is the only layer allowed to compile ComfyUI's official local LTX 2.5 T2V/I2V/FLF2V/Ingredients templates and submit them to the isolated loopback server.
...
Model-enum reconciliation must preserve semantic roles as well as filenames. In particular, `prompt_enhance_model` may resolve only to a dedicated `gemma4_e2b_it_*` checkpoint and never to the main `with-proj-ltx-2.5` encoder. A sole role-compatible enhancer variant may replace a stale template precision suffix; do not generalize that exception to other model roles. Keep ambiguous substitutions invalid and covered by fixture-only compiler tests.

Director mode requires the public `PromptRelayEncode` node, Lightricks' exact LTX 2.5 Ingredients workflow, and the official Ingredients IC-LoRA model. Resolve all dependency paths and model identities server-side. The browser may provide only normalized global/segment text, bounded durations/settings, and a server-created private sheet identity. Recursively compile nested subgraphs, identify patch points by unique class plus verified consumer relationship, and fail visibly on missing or ambiguous structure. Never silently fall back, claim frame-level control, combine Director with Blender, or claim the standard resolution preset while the official workflow derives geometry from the sheet. Tests are pure fixtures only; a real Director enqueue requires explicit user direction and an idle GPU.
...
Blender Auto-Pilot is a third fixed-purpose adapter. `lib/blender-autopilot.mjs` owns the scene-spec schema, loopback Ollama/CWM URL rules, and job contract. `lib/cwm-setup.mjs` owns official Code World Model identity (`facebook/cwm`), the required think system prompt, Hugging Face probe, and loopback OpenAI-server detection. `scripts/ltx-autopilot-runner.py` is the only orchestrator allowed to call loopback CWM or Ollama, spawn Blender, and optionally reuse `ltx-create-runner.py` for official FLF2V clothing. Prefer CWM when a loopback `/v1/models` server or an Ollama model named `cwm` is available. CWM is research-only and non-commercial; never auto-accept its license, never log Hugging Face tokens, and never execute CWM-generated Python. Official facebook/cwm needs about 80 GB VRAM; do not claim a 16 GB GPU can load it. `scripts/blender-autopilot.py` may only build allowlisted kits from that spec. The local planner may emit JSON, never Python/`bpy`/shell. Keep Ollama on `127.0.0.1`/`localhost`, unload it before GPU recording/clothing, and never save over a seed `.blend`. LTX clothing is appearance-only from Blender first/last frames plus identity locks; do not call it physics-preserving. The Final Override intro preset may lock geometry while still allowing appearance rewrites. Tests use the pure spec/job contract and `--validate-job` only—never real Ollama, Blender, n8n, Docker, or ComfyUI.
