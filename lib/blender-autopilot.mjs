...
import { normalizeCwmUrl } from './cwm-setup.mjs';
...
export function chooseOllamaModel(installed = [], preferred = '') {
  const names = (Array.isArray(installed) ? installed : []).map((item) => String(item || '').trim()).filter(Boolean);
  const set = new Set(names);
  if (preferred && set.has(preferred)) return preferred;
  const cwm = names.find((name) => /(^|[:/_-])cwm([:-]|$)/i.test(name) || name.toLowerCase().includes('facebook/cwm'));
  if (cwm) return cwm;
  return AUTOPILOT_PREFERRED_MODELS.find((name) => set.has(name)) || names[0] || null;
}

export function looksLikeFinalOverrideIntro(...parts) {
  const hay = parts.map((part) => String(part || '')).join(' ').toLowerCase();
  if (!hay.trim()) return false;
  if (hay.includes('final override') && (hay.includes('earth') || hay.includes('halo') || hay.includes('cathedral'))) return true;
  return hay.includes('earth') && hay.includes('halo') && hay.includes('cathedral') && (hay.includes('biodome') || hay.includes('bio-dome') || hay.includes('moon'));
}
...
export function autopilotCapability({
  ...
  const plannerReady = Boolean((cwmOnline && cwmModel) || (ollamaOnline && ollamaModel));
  const planner = cwmOnline && cwmModel ? 'cwm' : ollamaOnline && ollamaModel ? 'ollama' : null;
  const planningReady = Boolean(preparationReady && plannerReady);
  return {
    schemaVersion: AUTOPILOT_SCHEMA_VERSION,
    preparationReady,
    planningReady,
    planner,
    plannerModel: cwmOnline ? cwmModel : ollamaModel || null,
    ...
    blockedReason: !blenderInstalled
      ...
      : !adapterInstalled || !runnerInstalled
        ...
        : !plannerReady
          ? 'No local planner is online. Connect Code World Model on loopback, or keep Ollama running. The Final Override intro preset can still build without a planner.'
          : planner === 'cwm'
            ? 'Code World Model will plan a schema-validated scene. Blender owns camera and blocking. LTX may only clothe appearance and add smaller motion.'
...
export function buildAutopilotJob(input = {}) {
  ...
  const job = {
    ...
    cwmUrl: input.cwmUrl ? normalizeCwmUrl(input.cwmUrl) : '',
    cwmModel: phrase(input.cwmModel, 160),
...
export function validateAutopilotJob(job = {}) {
  ...
  if (job.cwmUrl) job.cwmUrl = normalizeCwmUrl(job.cwmUrl);
  if (job.clothWithLtx) {
