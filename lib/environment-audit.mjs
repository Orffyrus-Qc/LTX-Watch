...
export const OFFICIAL_LINKS = Object.freeze({
  ...
  comfyManager: 'https://docs.comfy.org/manager/overview',
  comfyManagerInstall: 'https://docs.comfy.org/manager/install',
  comfyManagerRepository: 'https://github.com/Comfy-Org/ComfyUI-Manager',
  ...
  sam3License: 'https://huggingface.co/facebook/sam3.1',
...
async function findBlenderInstallation() {
...
async function getComfyBlenderAudit(comfyRoot, customNodeRoot, comfyUrl, receipt) {
  ...
  const configured = Boolean(
    ...
    && receipt?.serverAddress === normalizedServer
    && receipt?.version === addonVersion
    && receipt?.version === customNodesVersion
    && receipt?.blenderVersion === blender?.version.text
  );
...
async function getPythonAudit(comfyRoot) {
  ...
  if (!executable) return { installed: false, version: null, pipVersion: null, pipHealthy: false, pipMessage: 'No ComfyUI Python environment was detected.', packages: [], localRequirements: null, latestRequirements: null, managerRequirements: null, managerPackageVersion: null };
  ...
  const managerRequirementsText = await readFile(path.join(comfyRoot, 'manager_requirements.txt'), 'utf8').catch(() => '');
  const latestRequirementsText = await fetchText('https://raw.githubusercontent.com/Comfy-Org/ComfyUI/master/requirements.txt');
  const selectedNames = new Set(['torch', 'torchvision', 'torchaudio', 'transformers', 'numpy', 'comfyui-frontend-package', 'comfyui-workflow-templates', 'comfyui-embedded-docs', 'comfy-kitchen', 'comfyui-manager']);
  return {
    ...
    managerRequirements: managerRequirementsText ? analyzeRequirements(managerRequirementsText, packages) : null,
    managerPackageVersion: packages.find((item) => normalizedPackageName(item.name) === 'comfyui-manager')?.version || null,
  };
}
...
async function getRunnerProfile(config, render) {
  ...
  const signals = {
    ...
    managerEnabled: /["']--enable-manager["']|\s--enable-manager(?:\s|$)/.test(runnerText),
    secondaryLtxEnabled: plan?.gpu1?.ltx_enabled === true,
  };
  const safeguards = Object.entries(signals).filter(([key, value]) => !['secondaryLtxEnabled', 'managerEnabled'].includes(key) && value).map(([key]) => key);
  const managerSetupMode = signals.managerEnabled
    ? 'configured'
    : /args\.server_extra_args\s*=\s*\[\s*\]/.test(runnerText)
      ? 'recognized-launcher'
      : runnerText ? 'manual-flag-required' : 'unavailable';
  return {
    externalRunner: Boolean(runnerText),
    safeguards,
    secondaryLtxEnabled: signals.secondaryLtxEnabled,
    managerEnabled: signals.managerEnabled,
    managerSetupMode,
...
async function getComfyManagerAudit(comfyRoot, managerRepo, python, runnerProfile) {
  const requirementsPath = path.join(comfyRoot, 'manager_requirements.txt');
  const cliArgsPath = path.join(comfyRoot, 'comfy', 'cli_args.py');
  const [requirementsText, cliArgsText] = await Promise.all([
    readFile(requirementsPath, 'utf8').catch(() => ''),
    readFile(cliArgsPath, 'utf8').catch(() => ''),
  ]);
  const builtinAvailable = Boolean(requirementsText && /--enable-manager/.test(cliArgsText));
  const managerRequirements = python.managerRequirements || analyzeRequirements(requirementsText, python.packages || []);
  const packageVersion = python.managerPackageVersion || null;
  const dependenciesReady = Boolean(builtinAvailable && managerRequirements.satisfied);
  const launchFlagConfigured = Boolean(runnerProfile.managerEnabled);
  const legacyInstalled = Boolean(managerRepo.installed);
  const builtInReady = Boolean(builtinAvailable && dependenciesReady && launchFlagConfigured && !legacyInstalled);
  const updateAvailable = Boolean(legacyInstalled && managerRepo.updateStatus === 'behind');
  const state = builtInReady
    ? 'ready'
    : builtinAvailable && legacyInstalled
      ? 'migration-available'
      : builtinAvailable && !dependenciesReady
        ? 'dependencies-required'
        : builtinAvailable && !launchFlagConfigured
          ? 'activation-required'
          : legacyInstalled
            ? updateAvailable ? 'update-available' : 'legacy-ready'
            : 'core-update-required';
  const detail = builtInReady
    ? `Built-in Manager ${packageVersion || ''} is installed and enabled for the detected launcher.`.replace(/\s+is/, ' is')
    : builtinAvailable && legacyInstalled
      ? 'Legacy custom-node Manager detected. A guarded migration to the built-in Manager is available.'
      : builtinAvailable && !dependenciesReady
        ? 'Built-in Manager is supported; its official Python dependency still needs to be installed.'
        : builtinAvailable && !launchFlagConfigured
          ? 'Built-in Manager is installed but the detected ComfyUI launcher still needs --enable-manager.'
          : legacyInstalled
            ? updateAvailable ? `Legacy Manager is installed and ${managerRepo.behindBy} commit${managerRepo.behindBy === 1 ? '' : 's'} behind.` : 'Legacy Manager is installed; update ComfyUI core to migrate to the built-in Manager.'
            : 'This ComfyUI revision does not expose the built-in Manager setup files.';
  return {
    ready: builtInReady || Boolean(legacyInstalled && !builtinAvailable),
    state,
    detail,
    mode: builtInReady ? 'built-in' : legacyInstalled ? 'legacy-custom-node' : builtinAvailable ? 'built-in-pending' : 'missing',
    builtinAvailable,
    dependenciesReady,
    packageVersion,
    launchFlagConfigured,
    launcherSetupMode: runnerProfile.managerSetupMode,
    legacyInstalled,
    legacyDirty: Boolean(managerRepo.dirty),
    updateAvailable,
    behindBy: managerRepo.behindBy || 0,
    automatedSetupSupported: Boolean(builtinAvailable && !managerRepo.dirty && ['configured', 'recognized-launcher'].includes(runnerProfile.managerSetupMode)),
    docsUrl: OFFICIAL_LINKS.comfyManagerInstall,
    projectUrl: OFFICIAL_LINKS.comfyManagerRepository,
  };
}
...
export async function buildEnvironmentAudit(config, render = { active: false, worker: false, comfyRunning: 0 }, options = {}) {
  ...
  const [managerRoot, ggufRoot, ltxCustomRoot, comfyBlenderRoot] = await Promise.all([
    findCustomNode(customRoot, 'ComfyUI-Manager'),
  ...
  const [python, gpuResult, disk, runnerProfile, comfyRepo, managerRepo, ggufRepo, comfyBlender] = await Promise.all([
    ...
    getRepositoryAudit(managerRoot, { id: 'manager', name: 'ComfyUI Manager', upstream: 'Comfy-Org/ComfyUI-Manager', branch: 'main' }),
  ...
  const comfyManager = await getComfyManagerAudit(comfyRoot, managerRepo, python, runnerProfile);
  const repositories = [comfyRepo, managerRepo, ggufRepo].filter((repo) => repo.installed);
  ...
  const checks = [
    ...
    statusItem('manager', 'ComfyUI Manager', comfyManager.ready, comfyManager.detail, comfyManager.ready ? 'ready' : 'attention'),
  ...
  if (comfyManager.state === 'migration-available') warnings.push('The legacy ComfyUI Manager can be migrated to the built-in Manager after the renderer and queue are idle.');
  if (comfyManager.legacyDirty) warnings.push('The legacy ComfyUI Manager contains local changes and will not be migrated automatically.');
  ...
  return {
    ...
    tools: {
      sam3: {
        ...
        links: { model: OFFICIAL_LINKS.sam3, license: OFFICIAL_LINKS.sam3License },
      },
      ltxAdvanced: {
